...
// =============================================================================
// The `convert` function rewrites a previous response's chat text (+ the
// recording's Attached Context block) into a single §2 `agent_prompt` artifact
// block. The server OWNS this text — no client field influences it (the same
// §14.1 control as `generate`'s prompt).
//
...
const CONVERT_PROMPT_V1 = `You convert an explanation into an instruction prompt for an AI coding/assistant agent. Your input is:
...
/** The complete conversion system prompt. A function (not the const) so the
 *  call sites mirror generate's composedSystemPrompt() shape. */
export function conversionSystemPrompt(): string {
  return CONVERT_PROMPT_V1;
}
