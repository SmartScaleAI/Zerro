...
export const CODE_TTL_SECONDS = optionalEnvInt(
  "TRIAL_CODE_TTL_SECONDS",
  10 * 60,
); // 10 min
...
export const TRIAL_TOKEN_TTL_SECONDS = optionalEnvInt(
  "TRIAL_TOKEN_TTL_SECONDS",
  30 * 60,
);

// Mailbox proof must survive a normal onboarding pause or app relaunch without
// becoming a generation credential. This token can only activate the Managed
// trial for its opaque onboarding-contact id; BYOK ignores it after the contact
// has been recorded.
export const ONBOARDING_CONTACT_TOKEN_TTL_SECONDS = optionalEnvInt(
...
export const TRIAL_RATE_LIMIT_PER_EMAIL = optionalEnvInt(
  "TRIAL_RATE_LIMIT_PER_EMAIL",
  8,
);
// Requests per IP per window — higher (shared NATs) but still bounds bulk abuse.
export const TRIAL_RATE_LIMIT_PER_IP = optionalEnvInt(
...
export const TRIAL_SEND_LIMIT_PER_EMAIL = optionalEnvInt(
...
export const TRIAL_DEVICE_BINDING_ENABLED = optionalEnvBool(
...
export const TRIAL_EMAIL_FROM = optionalEnv(
  "TRIAL_EMAIL_FROM",
  "Zerro <noreply@getzerro.app>",
);
