...
struct MenuBarPanelView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            MenuRow(label: "Quit VisualFlow AI", trailing: .hotkey("\u{2318}Q")) {
...
private struct MenuRow: View {
    ...
    private var labelColor: Color {
        ...
        return isSelected ? Color.white : Color.vfTextPrimary
    }

    private var trailingColor: Color {
        if isDisabled { return Color.vfTextTertiary.opacity(0.6) }
        return isSelected ? Color.white.opacity(0.85) : Color.vfTextTertiary
    }
    ...
    var body: some View {
        Button(action: action) {
            ...
            .background(isSelected ? Color.vfBrandBlue : Color.clear)
        }
        .buttonStyle(.plain)
        .disabled(isDisabled)
        .onHover { isHovered = $0 }
...
private struct PasteLastPromptRow: View {
    ...
    var body: some View {
        Button {
            ...
            VStack(alignment: .leading, spacing: 1) {
                HStack(spacing: 0) {
                    Text("Paste last prompt")
                        .font(.system(size: 13))
                        .foregroundStyle(isHovered ? Color.white : Color.vfTextPrimary)
                        .fixedSize()
                    Spacer(minLength: VFSpacing.lg)
                    Text("\u{2303}\u{2318}V")
                        .font(.system(size: 12))
                        .foregroundStyle(isHovered ? Color.white.opacity(0.85) : Color.vfTextTertiary)
                        .fixedSize()
                }
                Text("Polish the Pulse login form\u{2026}")
                    .font(.system(size: 11))
                    .foregroundStyle(isHovered ? Color.white.opacity(0.8) : Color.vfTextSecondary)
                    .lineLimit(1)
                    .truncationMode(.tail)
            }
            .padding(.horizontal, 10)
            ...
            .frame(maxWidth: .infinity, alignment: .leading)
            .contentShape(Rectangle())
            .background(isHovered ? Color.vfBrandBlue : Color.clear)
        }
        .buttonStyle(.plain)
        .onHover { isHovered = $0 }
    }
