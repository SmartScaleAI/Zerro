...
enum OpenAIClient {
    ...
    /// Authenticates a URLRequest by adding the Bearer header. Returns
    /// nil if no key is set so the caller can throw `missingAPIKey`.
    /// Mutating in place keeps multipart-body callers from rebuilding
    /// the entire request.
