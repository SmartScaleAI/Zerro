...
//  ZerroTests
//
//  Multi-model — the capture toolbar's frame math after the model chip
//  landed between the mode toggle and the mic chip. The toolbar is not a
//  SwiftUI HStack: every control's rect comes from static frame helpers
//  that the view renders with AND the controller hit-tests against, so
//  an arithmetic slip here means overlapping chrome and clicks landing
//  on the wrong control. These tests pin the invariants:
//    • cluster order mode → model → mic → record, gap-separated,
...
final class AreaSelectorToolbarLayoutTests: XCTestCase {
    ...
    private var mode: CGRect { AreaSelectorView.modeToggleFrame(forSelection: selection, in: bounds) }
    ...
    func testToolbarWidthIncludesModelChip() {
        // mode + gap + model + gap + mic + gap + record — derived from the
        // segment frames so the assertion tracks the constants.
        let gap = model.minX - mode.maxX
        let expected = mode.width + gap + model.width + gap + mic.width + gap + record.width
    ...
    func testClusterOrderIsModeModelMicRecord() {
        let gap = model.minX - mode.maxX
        XCTAssertGreaterThan(gap, 0, "model chip must sit AFTER the mode toggle with a gap")
        XCTAssertEqual(mode.minX, toolbar.minX)
        XCTAssertEqual(model.minX, mode.maxX + gap, accuracy: 0.001)
        XCTAssertEqual(mic.minX, model.maxX + gap, accuracy: 0.001,
                       "mic chip must shift right by the model chip's width + gap")
    ...
    func testSegmentsAreDisjointAndInsideToolbar() {
        let frames = [mode, model, mic, record]
    ...
    func testCenterPointsHitTheRightControl() {
        let centers = [mode, model, mic, record].map { CGPoint(x: $0.midX, y: $0.midY) }
        let frames = [mode, model, mic, record]
