# Zerro prompt v2 — in-repo mirror (locked 2026-06-11)
...
the in-repo mirror that Phases 3–4 propagate from into
`PromptGenerationSystemPrompt.swift`, `supabase/functions/generate/prompt.ts`,
and the `eval-models.mjs` prompt mirror — byte-identical, no re-wrapping.
...
changelog entry there. To eval against this file:
`--system-prompt Scripts/artifact-eval/prompt-v2.md` (the harness extracts the
first fenced block from `.md` files; a `.txt` path is read raw).
