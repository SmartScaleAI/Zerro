...
enum PromptGenerationSystemPrompt {
    ...
    /// The composed system prompt. No arguments since the typed-artifact
    /// refactor — kept as a function so call sites read the same as the
    /// server's `composedSystemPrompt()`.
