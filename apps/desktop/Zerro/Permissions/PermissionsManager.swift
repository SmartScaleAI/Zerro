...
final class PermissionsManager {
    ...
    /// Live "is Screen Recording currently usable" check: the primary
    /// CGPreflight plus the dev-time CGWindowList second opinion. Static +
    /// nonisolated so the capture-failure path (AppState.failureReason)
    /// can consult the same canonical logic when deciding whether a
    /// mid-session capture stop was a permission revocation — without
    /// holding a PermissionsManager instance. (Without sharing this, the
    /// failure path would rely on raw CGPreflight, which false-negatives
    /// under dev codesign-identity drift and would misreport a mic
    /// revocation as a screen-recording one.)
    ///
    /// CGWindowList rationale: with screen recording granted, foreign-
    /// process window dicts carry real `kCGWindowName` values; without it
    /// they're blank/nil. We inspect windows owned by OTHER processes
    /// (own-pid names are visible regardless) and treat any non-empty
    /// name as proof of permission.
