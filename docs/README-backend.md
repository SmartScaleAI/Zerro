...
| `GENERATE_DEV_TRANSCRIBE_FALLBACK_CREDITS` | optional | Credits charged for a metered **trial** Dev-Mode call-1 transcription when STT is somehow unpriced (default `1`; X-01/B-03). The normal charge is the real absorbed Whisper cost (`ceil(stt_usd / $0.01)`, floor 1). Subscription tokens are not charged for call 1. |
