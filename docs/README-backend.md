...
| `LS_VARIANT_MANAGED` | ⚠️ **A-01** | **Comma-separated list of ALL managed variant ids** — monthly AND yearly (e.g. `LS_VARIANT_MANAGED="1735329,1735330"`). `LS_VARIANT_YEARLY` is validated as a **subset** of this at startup, and a subscription whose variant is in NEITHER list logs an `unrecognized_variant` warn (a new variant shipped without config). |
| `LS_VARIANT_YEARLY` | ⚠️ **A-01 (required + validated)** | Comma-separated ids of the **yearly** variants. Sets `subscriptions.billing_interval`, which gates the yearly credit-refresh cron — so an empty/wrong value silently records yearly subs as `monthly` and **starves prepaid yearly customers after one month**. The webhook now **fails loud at startup**: it must be non-empty AND a subset of `LS_VARIANT_MANAGED`, or `subscription_created`/`subscription_updated` events return a distinct `500 config error: …` (logged + a `webhook_config_invalid` PostHog alarm) and are retried by LS until the env is fixed and the function redeployed. A mapped variant not listed here records `monthly`; a missing variant id records `NULL`. |
...
| `CONVERT_RATE_LIMIT_PER_IDENTITY` / `_WINDOW_SECONDS` | optional | `convert` per-identity rate limit (defaults `10` per `60`s). Input caps are `CONVERT_MAX_{PAYLOAD_BYTES,SOURCE_CHARS,CONTEXT_CHARS}`. For a **subscription** token this rate limit is the only quantitative gate (the endpoint stays free — they already pay). |
| `CONVERT_SLOT_STALE_SECONDS` / `CONVERT_IDEMPOTENCY_TTL_SECONDS` | optional | X-01 trial metering: `convert` charges a **trial** token against the same per-grant pool `generate` spends (cheapest model only). These mirror the generate slot stale-reclaim window + idempotency TTL (defaults `180`s / `900`s) so the metered trial path is cap-1 safe and a same-key retry replays instead of double-charging. Subscription behavior is unchanged. |
